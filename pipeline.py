                        ...
                        if payload_digest is None:
                            ...
                            if b'\r\n\r\n' in data:
                                ...
                                if b'transfer-encoding: ' in http_headers_raw.lower():
                                    chunked = re.search(br'(?im)^transfer-encoding:[ \t]*chunked\r?$', http_headers_raw) is not None
                                    has_payload = chunked
                                if not chunked:
...
LUA_SHA1 = get_hash(os.path.join(CWD, 'navertv.lua'))
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'video':
                ...
                wget_args.extend(['--warc-header', 'navertv-playlist: '+item_value])
                wget_args.append('https://apis.naver.com/now_web2/now_web_api/v1/playlist/'+quote(item_value))
            elif item_type == 'search':
                wget_args.extend(['--warc-header', 'navertv-search: '+item_value])
                wget_args.append('https://apis.naver.com/now_web2/now_web_api/v1/search?q='+quote(unquote(item_value))+'&sortKey=rel&page=1&pageSize=50')
            elif item_type == 'media':
                wget_args.extend(['--warc-header', 'navertv-media: '+item_value])
            ...
            if item_type != 'media':
                ...
                md = str(base64.b64encode(hmac.new(
                    ...
                    (wget_args[-1][:255]+msgpad).encode('utf8'), hashlib.sha1
                ).digest()), 'utf8')
                if '?' in wget_args[-1]:
                    wget_args[-1] += '&'
                else:
                    wget_args[-1] += '?'
                wget_args[-1] += urlencode({'msgpad': msgpad, 'md': md})
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/c/cd/Navertv-icon.png" height="50px" title=""/>
        <h2>Naver TV <span class="links"><a href="https://tv.naver.com/">Website</a> &middot; <a href="https://tracker.archiveteam.org/navertv/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Naver_TV">Wiki</a></span></h2>
        <p>Archiving Naver TV.</p>
    ''',
    utc_deadline=datetime.datetime(2026, 9, 30, 6, 0, 0)
)
